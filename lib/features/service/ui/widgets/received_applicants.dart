...
class ReceivedApplicants extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // If a service is provided, render a single "open" style card (same
    // layout as OpenApplicantsWidget's item). This lets callers (like the
    // community detail page) reuse this widget to show a service-card when
    // requestState == 1.
    if (service != null) {
      final s = service!;
      final skillNames =
      ...
      final location = (s.place.trim().isNotEmpty) ? s.place : (s.regionCode ?? '');
      final slots = s.slot;
