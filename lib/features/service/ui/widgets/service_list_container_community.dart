...
import '../../providers/service_pagination_provider_community.dart';
import 'service_card.dart';
import '../../data/model/service_filter.dart';

class ServiceListContainerCommunity extends ConsumerStatefulWidget {
  final String? userId;
  final String? query;
  ...
  const ServiceListContainerCommunity({
    super.key,
    this.userId,
    this.query,
    this.filter,
  ...
  @override
  ConsumerState<ServiceListContainerCommunity> createState() =>
      _ServiceListContainerCommunityState();
}

class _ServiceListContainerCommunityState extends ConsumerState<ServiceListContainerCommunity> {
  @override
  void initState() {
    super.initState();
    ...
    Future.microtask(() {
      ref
          .read(servicePaginationCommunityProvider(widget.userId).notifier)
          .fetchNextPage();
    });
  }
  ...
  Widget build(BuildContext context) {
    final paginationState = ref.watch(servicePaginationCommunityProvider(widget.userId));
    ...
    Widget listView = NotificationListener<ScrollNotification>(
      onNotification: (notification) {
        if (notification.metrics.pixels >=
          ...
          final notifier =
              ref.read(servicePaginationCommunityProvider(widget.userId).notifier);
          final state = ref.read(servicePaginationCommunityProvider(widget.userId));
    ...
    listView = RefreshIndicator(
      onRefresh: () async {
        final provider = servicePaginationCommunityProvider(widget.userId);
